...
class SH:   #提供以共享内存的方式向本机其他进程传递数据（效率最高）
    sharemem = mmap.mmap(0,25664,access=mmap.ACCESS_WRITE,tagname='__eeg_from_pycorder__')
    eegchs = 0
    point = 0
    addr = ('127.0.0.1',15831)
    
    #共享内存区写入数据格式
    # samplingrate,cnum,pnum,data   float64
    # cnum:通道数
    # pnum:一次采样的点数，即samplingrate/20 （50ms采样一次）
    
    def __init__(self):
        self.sock = socket.socket(socket.AF_INET,socket.SOCK_DGRAM)
    
    def put(self):
        self.sock.sendto('\x01',self.addr)
...
def InstantiateModules(run_as):
    ...
    if 'RC' in run_as:
        ...
        modules = [AMP_ActiChamp(shm),  #by mrtang   通过amp模块获取放大器设置参数
                   MNT_Recording(),
                   TRG_Eeg(),
                   StorageVision(), 
                   FLT_Eeg(shm),    #by mrtang   通过filter模块传递滤波后的数据
                   #TUT_0(),
                   #RDA_Server(),
                   IMP_Display(), 
                   DISP_Scope(instance=0)
...
class MainWindow(Qt.QMainWindow, frmMain.Ui_MainWindow):
    ...
    def writeBatteryLog(self, event):
        ...
        try:
                ...
                if event.type == EventType.STATUS:
                    ...
                    if event.status_field == "Battery": 
                        ...
                        if (t - self.battery_timer >= 59) and (self.battery_mode >= 0):
                            ...
                            logentry = "%.2f\t%s\t%s"%(voltage, event.event_time.strftime("%H:%M:%S\t%d/%m/%Y"), self.battery_ampSN)
            ...
            # enable battery log with command line option -rBL
            self.battery_log = (self.cmd_options.RunAs == "BL")
            self.battery_timer = time.clock() - 60.0
            self.battery_ampSN = "???"
            self.battery_rate = "???"
            self.battery_mode = -1
            ...
            #self.battery_logfile = os.path.join(os.path.dirname(os.path.realpath(__file__)), "CHampBattery.log")
            # log to users home /.PyCorder directory
            self.battery_logfile = os.path.join(unicode(Qt.QDir.toNativeSeparators(Qt.QDir.homePath())), "." + self.application_name,  "CHampBattery.log")
