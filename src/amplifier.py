...
class AMP_ActiChamp(ModuleBase):
    ...
    def __init__(self,sh,*args, **keys):  #revise by mrtang, added "sh" param
        ''' Constructor
        '''
        ModuleBase.__init__(self, name="Amplifier", **keys)
        
        self.shm = sh  # by mrtang
    ...
    def process_start(self):
        ...
        buf1 = np.array([samplerate],dtype=np.float64).tostring()
        self.shm.sharemem.seek(0)
        self.shm.sharemem.write(buf1)
        
        buf2 = np.array([pnum],dtype=np.float64).tostring()
        self.shm.sharemem.seek(16)
        self.shm.sharemem.write(buf2)
        # cum由滤波器进程来写入
        # 完成头信息写入后，将不再更改
        time.sleep(0.2)
