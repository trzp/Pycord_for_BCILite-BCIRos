...
class FLT_Eeg(ModuleBase):
    ...
    def _design_filter(self, frequency, type, slice):
        ...
        buf1 = np.array([self.shm.eegchs],dtype=np.float64).tostring()
        self.shm.sharemem.seek(8)
        self.shm.sharemem.write(buf1)
        # cum由滤波器进程来写入
        # 完成头信息写入后，将不再更改
    ...
    def process_output(self):
        ...
        buf = struct.pack('d',self.countindex)+self.data.eeg_channels.reshape(self.shm.eegchs*self.shm.point).tostring()
        self.shm.sharemem.seek(24)
        self.shm.sharemem.write(buf)
        self.shm.put()  #发射信号，告知客户进程可以安全读取数据
        return self.data
